...
def predict_drugCell(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features, drug_features):
	...
	np.savetxt(result_file+'/drugCell.txt', test_predict.cpu().numpy(), '%.4e')
...
print("batch: ")
predict_drugCell(predict_data, num_genes, drug_dim, opt.load, opt.hidden, opt.batchsize, opt.result, cell_mutation, drug_features)
