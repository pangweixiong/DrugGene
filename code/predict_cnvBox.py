...
def predict_cnvBox(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features, drug_features):
	...
	np.savetxt(result_file+'/cnvBox.txt', test_predict.cpu().numpy(), '%.4e')
...
print("batch: ")
predict_cnvBox(predict_data, num_genes, drug_dim, opt.load, opt.hidden, opt.batchsize, opt.result, cell_cn, drug_features)
