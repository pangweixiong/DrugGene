...
def predict_expBox(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features, drug_features):
	...
	# print(test_predict)
	np.savetxt(result_file+'/expBox.txt', test_predict.cpu().numpy(), '%.4e')
...
print("batch: ")
predict_expBox(predict_data, num_genes, drug_dim, opt.load, opt.hidden, opt.batchsize, opt.result, cell_exp, drug_features)
